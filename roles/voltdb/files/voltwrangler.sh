...
PARAM_LOCAL_IP_STRING="10.1.12.4,10.1.12.5,10.1.12.6"
KFACTOR=8
CMDLOGGING=YES
...
PRIV_IP="10.1.12.8"   # IP of "extra node"
NODECOUNT=3
...
echo "making sure chrony correct and in sync"
chronyc makestep
...
VEXEC=`ls /home/ubuntu/volt*/bin/voltdb`
PARAM_LOCAL_HOST=""


# Output directories - in case there aren't additional data drives
...
# If snapshot mountpoints exist - create folders
# Why are we doing this?  These already exist
# These are created by "filesystem.sh" during setup run
# Requires 2 nvme drives
if [ -d /voltdbdatassd2/voltdbroot ]
then
  SNAPSHOT_DIR=/voltdbdatassd2/voltdbroot/snapshot
  mkdir -p ${SNAPSHOT_DIR}
  chown -R ubuntu:ubuntu ${SNAPSHOT_DIR}
  rm -rf /voltdbdata/voltdbroot/snapshot
fi
...
then
  ...
  chown -R ubuntu:ubuntu ${AUTOSNAPSHOT_DIR}
  rm -rf /voltdbdata/voltdbroot/snapshots
fi
	
if [ -d /voltdbdatassd1/voltdbroot ]
then
  CMDLOG_DIR=/voltdbdatassd1/voltdbroot/cmdlog
  mkdir -p ${CMDLOG_DIR}
  chown -R ubuntu:ubuntu ${CMDLOG_DIR}
  rm -rf /voltdbdata/voltdbroot/cmdlog
else
  if [ -d /voltdbdatassd2/voltdbroot ]
  then
...
then
	...
	grafana-cli admin reset-admin-password inmemory
    ...
    # Disable Prometheus?
	for i in voltdbprometheusbl voltdbprometheus
	do
